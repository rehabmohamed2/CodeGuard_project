...
async function activate(context) {
    ...
    try {
        ...
        apiClient = new index_1.AnalysisAPIClient({
            context,
        ...
        if (firstAdminCreated) {
            vscode.window.showInformationMessage('First admin account created successfully! You now have full privileges. 🔐');
        }
...
function registerCommands(context) {
    context.subscriptions.push(vscode.commands.registerCommand('secure-code-analyzer.runAnalysis', async () => await runAnalysis()), vscode.commands.registerCommand('secure-code-analyzer.cancelAnalysis', async () => await cancelAnalysis()), vscode.commands.registerCommand('secure-code-analyzer.registerUser', async () => await registerNewUser()));
}
...
async function registerNewUser() {
    try {
        ...
        if (username && password && role) {
            await apiClient.registerUser(username, password, role);
...
async function runAnalysis() {
    ...
    try {
        ...
        const analysisId = await vscode.window.withProgress(progressOptions, async (progress, progressToken) => {
            ...
            const analysisId = await apiClient.analyzeFile(cppPath, abortController.signal);
            ...
            return monitorAnalysisProgress(analysisId, progress, progressToken, extensionToken.token);
        });
        // Final result handling
        if (currentAnalysisSession && currentAnalysisSession.id === analysisId) {
            await handleCompletedAnalysis(await apiClient.getAnalysisStatus(analysisId), cppPath);
        }
    }
    catch (error) {
...
async function monitorAnalysisProgress(analysisId, progress, progressToken, extensionToken) {
...
async function pollAnalysisStatus(analysisId, cppPath, token) {
...
async function handleCompletedAnalysis(status, cppPath) {
    try {
...
function handleFailedAnalysis(analysisId, error) {
...
function handleEditorChange(newFilePath) {
    if (currentAnalysisSession) {
...
async function generateSarifReport(results, cppPath) {
    const workspace = vscode.workspace.workspaceFolders?.[0];
    if (!workspace)
        throw new Error("No workspace open");
    const sarifExt = vscode.extensions.getExtension('MS-SarifVSCode.sarif-viewer');
    if (!sarifExt)
        throw new Error("SARIF Viewer not installed");
    try {
        ...
        await vscode.workspace.fs.writeFile(vscode.Uri.file(reportPath), Buffer.from(JSON.stringify(results[0], null, 2)));
