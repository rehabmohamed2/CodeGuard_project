...
export function parse(output: string): VulnerabilityLocation | null {
  const regex = /==\d+==ERROR: AddressSanitizer: (?:attempting )?(\S+)[\s\S]*?#\d+\s+0x[0-9a-f]+\s+in\s+(\S+)\s+([^\s:()]+):(\d+)(?::(\d+))?[\s\S]*?freed by thread[\s\S]*?#\d+\s+0x[0-9a-f]+\s+in\s+(\S+)\s+([^\s:()]+):(\d+)(?::(\d+))?[\s\S]*?allocated by thread[\s\S]*?#\d+\s+0x[0-9a-f]+\s+in\s+(\S+)\s+([^\s:()]+):(\d+)(?::(\d+))?/;
  const match = output.match(regex);

  if (!match || match[1] != 'double-free') return null;
